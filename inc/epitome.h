...
public:
	...
	inline int getEpitomeWidth(){ return eWidth; }
	inline int getEpitomeHeight(){ return eHeight; }
	inline int getEpitomePatchSideLength(){ return patchSideLengh; }
	inline int getEpitomePatchSpacing(){ return patchSpacing; }
	inline int getEpitomeNumIteration(){ return numIteration; }
	inline bool getEpitomeVerbose(){ return verbose; }
	inline bool getEpitomeRandomShifting(){ return RandomShifting; }
	inline bool getEpitomeIsGrayScale(){ return isgrayScale; }
	inline void setFileExtension(std::string & filename){
	...
	void learnImgGenreEpitome_plus(const vector<string> & v_imageFileNameList);

	void getMaxPostRowColIdx(
		const vector<string> & v_imageFileNameList, // read input images via their file-names.
		const int & patchSpac
		);
	...
	bool get_xPatch_unnormalPosterior_RGB_or_Gray_Channel(
	...
	bool get_xPatch_sumQXX_etc_RGB_or_Gray_Channel(
		// It's pretty much all written in the FFTW documentation about thread safety:
		// but some care must be taken because the planner routines share data
		// (e.g.wisdom and trigonometric tables) between calls and plans.
		// so here we set ws as a parameter,
		...
		//}; 
		vector<Epitome_Doulbe*> v_p_sumQ_sumQX_RGB_etc
		);

	void paral_getMaxPostRowColIdx(
		const vector<string> & v_imageFileNameList, // read input images via their file-names.
		const int & patchSpac);
