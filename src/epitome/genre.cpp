...
void ImgEpitome::learnImgGenreEpitome_plus(
	...
	std::cout << "learnImgGenreEpitome_plus" << endl;
	...
	//}; 
	vector<vector<Epitome_Doulbe*>> v_v_p_sumQ_sumQX_RGB_etc;
	if (isgrayScale){ /*gray */
		v_v_p_sumQ_sumQX_RGB_etc = vector<vector<Epitome_Doulbe*>>(MAX_PARALLEL_SIZE, vector<Epitome_Doulbe*>(3));
		...
		v_v_p_sumQ_sumQX_RGB_etc = vector<vector<Epitome_Doulbe*>>(MAX_PARALLEL_SIZE, vector<Epitome_Doulbe*>(7));
		for (int id = 0; id < MAX_PARALLEL_SIZE; ++id) {
			v_v_p_sumQ_sumQX_RGB_etc[id][0] = new Epitome_Doulbe[eNum]; // for p_sumQ, #0
...
#pragma omp critical
					printf("For image %d of %d 's patch learning, %f %% Complete...\n", current_image_index, 
...
#ifdef _DEBUG
#pragma omp master
			printf("Parallel calculation finishes. Now %d OpenMP threads were used.\n", nThreads);
#endif
	...
	// memory release time
	t = clock() - t;
	printf("It took %f seconds for memory release.\n", ((float)t) / CLOCKS_PER_SEC);


	if (isgrayScale){ /*gray epitome*/
	cv::Mat1d mat_eMean(eHeight, eWidth);
	...
	compression_params_png[0] = IMWRITE_PNG_COMPRESSION;
	...
	compression_params_jpeg[0] = IMREAD_UNCHANGED;
